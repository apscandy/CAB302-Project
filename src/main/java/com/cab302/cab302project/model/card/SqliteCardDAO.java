...
public final class SqliteCardDAO implements ICardDAO {
    ...
    private final String getSmartCardsSQL = "SELECT c.deck_id, c.id AS card_id, c.question, c.answer, c.tags, csrv.correct_count, csrv.incorrect_count, csrv.success_rate FROM card c LEFT JOIN card_success_rate_view csrv ON c.id = csrv.card_id WHERE c.deck_id = ? AND c.is_deleted = 0 ORDER BY COALESCE(csrv.success_rate, 0) ASC";
    ...
    @Override
    public void getCardsForDeckRandom(Deck deck) {
        ...
        try {
            con.setAutoCommit(false);
            try (PreparedStatement getCardStatment = con.prepareStatement(getRandomCardsSQL)) {
    ...
    @Override
    public void getCardsForDeckSmart(Deck deck) {
        if (deck == null || deck.getId() == 0) {
            throw new DeckIsNullException("Invalid deck");
        }
        List<Card> cards = new ArrayList<>();
        try {
            con.setAutoCommit(false);
            try (PreparedStatement getCardStatment = con.prepareStatement(getSmartCardsSQL)) {
                getCardStatment.setInt(1, deck.getId());
                ResultSet rs = getCardStatment.executeQuery();
                while (rs.next()) {
